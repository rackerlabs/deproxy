...
class Deproxy {
  ...
  public MessageChain makeRequest(
    ...
    def path = uri.path
    def rawPathQueryFragment = url.substring(url.indexOf(uri.authority) + uri.authority.length())
    ...
    def request = new Request(method, path, headers, requestBody, rawPathQueryFragment)
    //
  ...
  //    def send_request(self, scheme, host, request):
    ...
    def sendRequest(Request request, scheme, host, port = null) {
        ...
        def requestLine = String.format("%s %s HTTP/1.1", request.method, request.rawPathQueryFragment ?: "/")
        ...
        try {
            ...
            response = new Response(code, message, headers, body)
        } finally {
            if (reader) {
                reader.close()
            }
  ...
  def shutdown() {
    ...
    log.debug("Shutting down all endpoints")
